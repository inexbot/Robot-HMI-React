...
function Disern(props) {
  ...
  for (let i = 1; i < 10; i++) {
    visinoIDrNumchildren.push(<Option key={i}>{i}</Option>);
  }
  const DI_capturePosNumchildren = [];
  DI_capturePosNumchildren.push(<Option key="1">无</Option>);
  const globalVarNumchildren = [];
  for (let i = 1; i < 991; i++) {
    globalVarNumchildren.push(
      <Option key={i}>
        {i <= 9 ? "GB00" + i : i <= 99 ? "GB0" + i : "GB" + i}
      </Option>
    );
  }
  ...
  return (
    ...
    >
                ...
                if (Diserntype === 0) {
                  ...
                  dataList = {
                    ...
                    detectSrc: {
                      type: Number(Diserntype),
                      globalVar:
                        Number(signalSource) <= 9
                          ? "GB00" + signalSource
                          : Number(signalSource) <= 99
                          ? "GB0" + signalSource
                          : "GB" + signalSource,
                    },
                    identification: {
      ...
      <div className="connect" style={{ marginTop: 8 }}>
        <Table
          scroll={{
