  ...
  function SensorOne(props){
    ...
    return(
      <div className="backconnect" style = {{ height:document.body.clientHeight  * 0.68 }}>
        ...
        <p style={{ fontSize:"18px",margin:"0" ,lineHeight:"1",padding:"2px 0 2px 0" }}>1.准备一个带有尖端的工件，放置在传送带工作时的宽度处，并在机器人法兰上安装一个尖锥;</p>
        <p style={{ fontSize:"18px",margin:"0",lineHeight:"1",padding:"2px 0 2px 0"  }}>2.移动传送带，使工件移动经过传感器位置，触发IO，然后继续移动传送带，将工件移动到机器人的运动范围内的标定点，将机器人移动到工件处，使尖对准尖;</p>
        <p style={{ fontSize:"18px",margin:"0",lineHeight:"1",padding:"2px 0 2px 0"  }}>3.点击标定按钮;</p>
          <Table
            pagination={false}
            columns={columns}
            dataSource={data }
