...
const mapStateToProps = (state) => {
  ...
  function Vision(){
    ...
    return(
            <div>
                <HashRouter  >
                     <Steps current={current} onChange={onChange} style={{ position:"relative",top:"60px",zIndex:"2",width:"90%",margin:"0 auto" }}>
                       <Step title="视觉参数设置"  />
                       <Step title="视觉范围设置" />
                       <Step title="视觉位置设置" />
                       <Step title="位置调试"  />
                    ...
                    <Switch>
                      <Route exact path='/vision' component={Parameter}></Route>
                    </Switch>
                </HashRouter>
            </div>
    )
